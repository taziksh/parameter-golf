...
class Hyperparameters:
    ...
    # Causal-JEPA auxiliary loss. See experiments/cjepa/HYPOTHESIS.md.
    # Predictor + teacher are discarded at eval; not counted in artifact size.
    cjepa_enabled: bool = bool(int(os.environ.get("CJEPA_ENABLED", "1")))
    cjepa_k: int = int(os.environ.get("CJEPA_K", 4))
    cjepa_lambda: float = float(os.environ.get("CJEPA_LAMBDA", 0.5))
    cjepa_ema_decay: float = float(os.environ.get("CJEPA_EMA_DECAY", 0.99))
    cjepa_predictor_mult: int = int(os.environ.get("CJEPA_PREDICTOR_MULT", 2))
    cjepa_smooth_l1_beta: float = float(os.environ.get("CJEPA_SMOOTH_L1_BETA", 1.0))
    cjepa_every_n_steps: int = int(os.environ.get("CJEPA_EVERY_N_STEPS", 4))
    cjepa_monitor_every: int = int(os.environ.get("CJEPA_MONITOR_EVERY", 200))
...
def ema_update_mlx(teacher: nn.Module, student: nn.Module, decay: float) -> float:
    # teacher <- decay * teacher + (1 - decay) * student, in place on the module.
    # Returns the mean |after - before| on a probe param as a liveness check:
    # if this stays at 0, the update isn't propagating to compiled forwards.
    teacher_flat = dict(tree_flatten(teacher.parameters()))
    student_flat = dict(tree_flatten(student.parameters()))
    probe_key = next(iter(teacher_flat))
    probe_before = teacher_flat[probe_key].astype(mx.float32)
    new_flat = {
        k: (decay * teacher_flat[k].astype(mx.float32)
            + (1.0 - decay) * student_flat[k].astype(mx.float32)).astype(teacher_flat[k].dtype)
        for k in teacher_flat
    }
    teacher.update(tree_unflatten(list(new_flat.items())))
    probe_after = dict(tree_flatten(teacher.parameters()))[probe_key].astype(mx.float32)
    return float(mx.mean(mx.abs(probe_after - probe_before)).item())


def teacher_probe_pairwise_cosine_mlx(teacher_hidden: mx.array, sample: int = 256) -> float:
    # Cheap collapse detector. >0.5 is a warning signal (full or dimensional).
    flat = teacher_hidden.reshape(-1, teacher_hidden.shape[-1]).astype(mx.float32)
    n = flat.shape[0]
    if n > sample:
        idx = mx.random.permutation(n)[:sample]
        flat = flat[idx]
        n = sample
    norm = mx.rsqrt(mx.sum(flat * flat, axis=-1, keepdims=True) + 1e-8)
    flat = flat * norm
    sim = flat @ flat.T
    mask = 1.0 - mx.eye(n)
    off_diag_sum = mx.sum(sim * mask)
    denom = max(n * (n - 1), 1)
    return float((off_diag_sum / denom).item())


class CausalJEPAPredictor(nn.Module):
    # Position-wise 2-layer MLP that maps student hidden state at position i
    # to a prediction of the teacher hidden state at position i+k. Discarded
    # at eval (not saved in submission artifact).
...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, num_layers: int, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int,
                 ...
                 qk_gain_init: float, cjepa_enabled: bool = False, cjepa_predictor_mult: int = 2):
        super().__init__()
        ...
        self.cjepa_predictor = (
            CausalJEPAPredictor(dim, cjepa_predictor_mult) if cjepa_enabled else None
        )
    ...
    def __call__(self, input_ids: mx.array) -> mx.array:
        return self.forward_hidden(input_ids)

    def loss_from_hidden(self, hidden: mx.array, target_ids: mx.array) -> mx.array:
    ...
    def loss(self, input_ids: mx.array, target_ids: mx.array) -> mx.array:
        return self.loss_from_hidden(self.forward_hidden(input_ids), target_ids)
    ...
    def loss_with_cjepa(
        ...
        cjepa_k: int,
        cjepa_lambda: float,
        cjepa_smooth_l1_beta: float,
    ) -> mx.array:
        hidden = self.forward_hidden(input_ids)
        ce = self.loss_from_hidden(hidden, target_ids)
        if self.cjepa_predictor is None or cjepa_lambda <= 0.0 or cjepa_k <= 0:
            return ce
        t_len = hidden.shape[1] - cjepa_k
        pred = self.cjepa_predictor(hidden[:, :t_len, :])
        # Per-token LayerNorm on target (V-JEPA / Data2Vec anti-collapse trick).
        target_raw = mx.stop_gradient(teacher_hidden[:, cjepa_k:, :])
        mean = mx.mean(target_raw, axis=-1, keepdims=True)
        var = mx.mean((target_raw - mean) * (target_raw - mean), axis=-1, keepdims=True)
        target = (target_raw - mean) * mx.rsqrt(var + 1e-5)
        # Smooth-L1 (Huber). Matches V-JEPA's published loss.
        diff = pred.astype(mx.float32) - target.astype(mx.float32)
        abs_diff = mx.abs(diff)
        beta = cjepa_smooth_l1_beta
        cjepa = mx.mean(
            mx.where(abs_diff < beta, 0.5 * diff * diff / beta, abs_diff - 0.5 * beta)
        )
        return ce + cjepa_lambda * cjepa
...
class SplitOptimizers:
    ...
    def __init__(self, model: GPT, args: Hyperparameters):
        ...
        self.matrix_keys = [
            ...
            if (k.startswith("blocks.") or k.startswith("cjepa_predictor."))
            and p.ndim == 2
...
def loss_and_grad_chunked_cjepa(
    args: Hyperparameters,
    train_loader: TokenLoader,
    compiled_cjepa_loss_and_grad,
    compiled_teacher_hidden,
) -> tuple[mx.array, dict, mx.array | None]:
    chunk_sizes = token_chunks(args.microbatch_tokens, args.train_seq_len, args.mlx_max_microbatch_tokens)
    total_tokens = float(sum(chunk_sizes))
    loss_value = mx.array(0.0, dtype=mx.float32)
    grad_accum: dict[str, mx.array] | None = None
    last_teacher_hidden: mx.array | None = None
    for chunk_tokens in chunk_sizes:
        x, y = train_loader.next_batch(chunk_tokens, args.train_seq_len)
        teacher_hidden = mx.stop_gradient(compiled_teacher_hidden(x))
        loss, grads = compiled_cjepa_loss_and_grad(x, y, teacher_hidden)
        scale = float(y.size) / total_tokens
        loss_value = loss_value + loss.astype(mx.float32) * scale
        grad_accum = accumulate_flat_grads(grad_accum, grads, scale)
        if args.mlx_eager_eval:
            mx.eval(loss_value, grad_accum)
        last_teacher_hidden = teacher_hidden
    return loss_value, tree_unflatten(list(grad_accum.items())), last_teacher_hidden
...
def main() -> None:
    ...
    model = GPT(
        ...
        cjepa_enabled=args.cjepa_enabled,
        cjepa_predictor_mult=args.cjepa_predictor_mult,
    )
    opt = SplitOptimizers(model, args)

    # EMA teacher: same architecture as student *minus* the cJEPA predictor
    # (teacher never calls it). Built with cjepa_enabled=False so the predictor
    # params don't exist on the teacher at all — saves memory and EMA compute.
    # Student weights copied in with the predictor keys filtered out.
    cjepa_teacher: GPT | None = None
    if args.cjepa_enabled:
        cjepa_teacher = GPT(
            vocab_size=args.vocab_size,
            num_layers=args.num_layers,
            dim=args.model_dim,
        ...
        student_non_pred = {
            k: v for k, v in tree_flatten(model.parameters())
            if not k.startswith("cjepa_predictor.")
        }
        cjepa_teacher.update(tree_unflatten(list(student_non_pred.items())))
        teacher_n_params = sum(
            int(np.prod(p.shape)) for _, p in tree_flatten(cjepa_teacher.parameters())
        )
        log(f"cjepa_teacher_params:{teacher_n_params} (predictor excluded)")
    ...
    compiled_cjepa_loss_and_grad = None
    compiled_teacher_hidden = None
    if args.cjepa_enabled and cjepa_teacher is not None:
        def _cjepa_fn(x, y, teacher_hidden):
            return model.loss_with_cjepa(
                x, y, teacher_hidden, args.cjepa_k, args.cjepa_lambda, args.cjepa_smooth_l1_beta
            )
        compiled_cjepa_loss_and_grad = mx.compile(
            nn.value_and_grad(model, _cjepa_fn),
            inputs=model.state,
            outputs=model.state,
        )
        compiled_teacher_hidden = mx.compile(
            lambda x: cjepa_teacher.forward_hidden(x),
            inputs=cjepa_teacher.state,
            outputs=cjepa_teacher.state,
        )
    ...
    if args.warmup_steps > 0:
        ...
        # When cjepa is on we prime both compile paths (teacher forward + cjepa loss/grad) so the
        # main loop doesn't pay compile cost on its first cjepa step.
        for warmup_step in range(args.warmup_steps):
            accum: dict[str, mx.array] | None = None
            warmup_loss = mx.array(0.0, dtype=mx.float32)
            grad_scale = 1.0 / args.grad_accum_steps
            for _ in range(args.grad_accum_steps):
                if args.cjepa_enabled and compiled_cjepa_loss_and_grad is not None:
                    warmup_loss, grads, _ = loss_and_grad_chunked_cjepa(
                        args, train_loader,
                        compiled_cjepa_loss_and_grad, compiled_teacher_hidden,
                    )
                else:
    ...
    while True:
        ...
        run_cjepa = (
            args.cjepa_enabled
            and cjepa_teacher is not None
            and compiled_cjepa_loss_and_grad is not None
            and args.cjepa_every_n_steps > 0
            and (step % args.cjepa_every_n_steps == 0)
        )
        ...
        for _ in range(args.grad_accum_steps):
            if run_cjepa:
                loss, grads, last_teacher_hidden = loss_and_grad_chunked_cjepa(
                    args, train_loader,
                    compiled_cjepa_loss_and_grad, compiled_teacher_hidden,
                )
            else:
        ...
        ema_shift = 0.0
        if run_cjepa and cjepa_teacher is not None:
            ema_shift = ema_update_mlx(cjepa_teacher, model, args.cjepa_ema_decay)
        mx.synchronize()
        ...
        if args.train_log_every > 0 and (step <= 10 or step % args.train_log_every == 0 or stop_after_step is not None):
            suffix = f" cjepa:{'Y' if run_cjepa else 'N'}" if args.cjepa_enabled else ""
            log(
                f"step:{step}/{args.iterations} train_loss:{train_loss_value:.4f} "
                f"train_time:{approx_train_time_ms:.0f}ms step_avg:{approx_train_time_ms / step:.2f}ms "
                f"tok_s:{tok_s:.0f}{suffix}"
            )
        # Collapse monitor: fires on cJEPA steps at the configured cadence.
        # `step - 1` is the pre-increment step value — always divisible by
        # cjepa_every_n_steps on cJEPA steps, so divisibility by cjepa_monitor_every
        # (typically a multiple of cjepa_every_n_steps) lines up correctly.
        if (
            args.cjepa_enabled
            and last_teacher_hidden is not None
            and args.cjepa_monitor_every > 0
            and (step - 1) % args.cjepa_monitor_every == 0
        ):
            teacher_cos = teacher_probe_pairwise_cosine_mlx(last_teacher_hidden)
            log(f"step:{step} teacher_pairwise_cos:{teacher_cos:.4f} teacher_ema_shift:{ema_shift:.2e}")
    ...
    # Filter out cjepa_predictor and anything else that is training-only and
    # must not count against the 16MB submission artifact.
    flat_state = {
