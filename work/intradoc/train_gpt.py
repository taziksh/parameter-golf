...
# Token id used by `data/download_hf_docs_and_tokenize.py` to mark the start of
# every document in the packed shards.
BOS_TOKEN_ID = 1

# Maximum sub-documents per window. Each extra cu_seqlens slot adds about 2.5%
# to attention step time because the FA3 varlen kernel walks every slot. A
# value of 8 covers 99% of sp8192 windows; the rest merge surplus BOSes into
# the final sub-document.
MAX_SUBDOCS_PER_WINDOW = 8


def compute_intra_doc_meta_np(window_tokens, seq_len,
                              max_subdocs=MAX_SUBDOCS_PER_WINDOW,
                              bos_id=BOS_TOKEN_ID):
    """Builds intra-doc attention metadata for one training window.

    The model receives `window_tokens` as input. Within the window, attention
    must not cross document boundaries (positions of `bos_id`). This function
    derives the metadata FA3's varlen attention needs to enforce that, plus the
    per-document position ids RoPE consumes.

    Args:
        window_tokens: int64 numpy array of shape [seq_len]. The input tokens
            for one window (i.e. x[bi], not y[bi]).
        seq_len: Window length.
        max_subdocs: Maximum number of sub-documents to emit cu_seqlens entries
            for. Surplus BOS tokens are silently merged into the last sub-doc.
        bos_id: Token id treated as a document boundary.

    Returns:
        A pair (cu_seqlens, position_ids):
          cu_seqlens: int32 array of shape [max_subdocs + 1]. Sub-doc boundaries
            within the window. Trailing entries are padded to `seq_len`
            (zero-length sub-docs that FA3 skips), keeping the shape fixed.
          position_ids: int32 array of shape [seq_len]. Each token's offset from
            the start of its sub-doc, restarting at every BOS.

    Notes:
        The partial-doc-prefix at the window head (if any) is treated as its own
        sub-doc starting at index 0; its tokens get positions 0..first_bos-1.
        Overflow beyond max_subdocs is rare (0 occurrences in 488K windows
        sampled) but capped to avoid shape changes if it ever happens.
    """
    bos_positions = np.flatnonzero(window_tokens == bos_id)
    n_admitted = min(len(bos_positions), max_subdocs - 1)

    cu_seqlens = np.full(max_subdocs + 1, seq_len, dtype=np.int32)
    cu_seqlens[0] = 0
    if n_admitted > 0:
        cu_seqlens[1:1 + n_admitted] = bos_positions[:n_admitted].astype(np.int32)
        cu_seqlens[1 + n_admitted] = seq_len

    pos_arange = np.arange(seq_len, dtype=np.int32)
    if len(bos_positions) > 0:
        # Vectorised "position of most recent BOS at or before p": searchsorted
        # tells us how many BOSes precede each position; we take the previous
        # one. Tokens before the first BOS get -1, clamped to 0 so they index
        # from the window start (the partial-doc-prefix sub-doc).
        idx = np.searchsorted(bos_positions, pos_arange, side='right')
        last_bos_pos = np.where(
            idx > 0,
            bos_positions[np.maximum(idx - 1, 0)],
            -1,
        ).astype(np.int32)
        position_ids = pos_arange - np.maximum(last_bos_pos, 0)
    else:
        position_ids = pos_arange.copy()
    return cu_seqlens, position_ids


def compute_intra_doc_meta_gpu(x, max_subdocs=MAX_SUBDOCS_PER_WINDOW,
                               bos_id=BOS_TOKEN_ID):
    """GPU-side equivalent of compute_intra_doc_meta_np for batched inputs.

    Used by the eval, TTT, and GPTQ calibration paths whose token tensors
    already live on the device.

    Args:
        x: int tensor of shape [B, T] (typically int64).

    Returns:
        A pair (cu_seqlens [B, max_subdocs + 1] int32,
                position_ids [B, T] int32). Same semantics as the numpy
        helper: trailing cu_seqlens entries pad to T; positions reset at
        each BOS; the partial-doc-prefix at the window head counts as its
        own sub-doc starting at 0.
    """
    B, T = x.shape
    device = x.device
    is_bos = x == bos_id

    pos_arange = torch.arange(T, device=device, dtype=torch.int32).unsqueeze(0).expand(B, T)
    bos_or_neg1 = torch.where(is_bos, pos_arange, torch.full_like(pos_arange, -1))
    last_bos_pos, _ = bos_or_neg1.cummax(dim=1)
    position_ids = (pos_arange - last_bos_pos.clamp_min(0)).to(torch.int32)

    # Sort a per-token tensor whose value is the BOS position when the token
    # IS a BOS, and a sentinel (>= T) otherwise. The first (max_subdocs - 1)
    # sorted values are the smallest BOS positions in the window.
    sentinel = T + 1
    candidates = torch.where(is_bos, pos_arange, torch.full_like(pos_arange, sentinel))
    sorted_cands, _ = candidates.sort(dim=1)
    admitted = sorted_cands[:, :max_subdocs - 1]
    admitted = torch.where(admitted >= T, torch.full_like(admitted, T), admitted)

    cu_seqlens = torch.empty(B, max_subdocs + 1, dtype=torch.int32, device=device)
    cu_seqlens[:, 0] = 0
    cu_seqlens[:, 1:max_subdocs] = admitted.to(torch.int32)
    cu_seqlens[:, max_subdocs] = T
    return cu_seqlens, position_ids
...
class ShuffledSequenceLoader:
	...
	def next_batch(self, global_tokens, grad_accum_steps):
		...
		cu_seqlens_np = np.empty(
			(device_batch_size, MAX_SUBDOCS_PER_WINDOW + 1), dtype=np.int32
		)
		position_ids_np = np.empty((device_batch_size, self.seq_len), dtype=np.int32)
		...
		for bi in range(device_batch_size):
			...
			window_np = np.array(mm[start_ind:start_ind + self.seq_len + 1], dtype=np.int64)
			window = torch.as_tensor(window_np)
			x[bi] = window[:-1]
			y[bi] = window[1:]
			# Mask metadata is derived from x, not y, since attention reads x.
			cu_seqlens_np[bi], position_ids_np[bi] = compute_intra_doc_meta_np(
				window_np[:-1], self.seq_len
			)

		cu_seqlens = torch.as_tensor(cu_seqlens_np)
		position_ids = torch.as_tensor(position_ids_np)
		return (
			x.to(self.device, non_blocking=True),
			y.to(self.device, non_blocking=True),
			cu_seqlens.to(self.device, non_blocking=True),
			position_ids.to(self.device, non_blocking=True),
		)
...
class Rotary(nn.Module):
	"""Rotary positional embeddings via a precomputed cos/sin lookup table.

	Each token's RoPE position is looked up by ``position_ids`` rather than
	derived from its index in the sequence. This lets positions reset at
	document boundaries within a packed window, matching the intra-doc
	attention mask.
	"""

	def __init__(self, head_dim, base=1e4, max_pos=2048, rope_dims=0):
		super().__init__()
	...
	def forward(self, position_ids, dtype):
		"""Gather cos/sin for each (batch, token) position.

		Args:
			position_ids: int tensor of shape [B, T]. Values must be in
				[0, max_pos).
			dtype: dtype to cast the gathered cos/sin to (typically the Q/K
				dtype, e.g. bf16).

		Returns:
			A pair (cos, sin), each of shape [B, T, 1, rope_dims // 2],
			broadcasting across heads in apply_rotary_emb.
		"""
...
class CausalSelfAttention(nn.Module):
	def __init__(self, dim, num_heads, num_kv_heads, rope_base, qk_gain_init, train_seq_len):
		...
		self.rope_dims = 0
		self.rotary = Rotary(self.head_dim, base=rope_base, max_pos=train_seq_len)
		self.use_xsa = False
	...
	def forward(self, x, cu_seqlens, position_ids):
		"""Intra-doc-masked attention via FA3 varlen.

		Args:
			x: input activations of shape [B, T, dim].
			cu_seqlens: int32 tensor of shape [B, MAX_SUBDOCS_PER_WINDOW + 1].
				Per-window sub-doc boundaries; trailing entries equal T to
				represent zero-length sub-docs (FA3 skips them).
			position_ids: int tensor of shape [B, T]. Per-token position within
				its sub-doc, used to gather RoPE cos/sin.
		"""
		B, T, dim = x.shape
		...
		# Flatten across batch for varlen and offset cu_seqlens by b*T per
		# window so the kernel sees one packed sequence of B*T tokens with
		# B*MAX_SUBDOCS_PER_WINDOW sub-documents end-to-end.
		q_flat = q.reshape(B * T, self.num_heads, self.head_dim)
		k_flat = k.reshape(B * T, self.num_kv_heads, self.head_dim)
		v_flat = v.reshape(B * T, self.num_kv_heads, self.head_dim)
		batch_offsets = torch.arange(B, device=x.device, dtype=torch.int32) * T
		cu_flat = (cu_seqlens + batch_offsets[:, None]).reshape(-1)

		y = flash_attn_varlen_func(
			q_flat, k_flat, v_flat,
			cu_seqlens_q=cu_flat,
			cu_seqlens_k=cu_flat,
			max_seqlen_q=T,
			max_seqlen_k=T,
			causal=True,
		)
...
class Block(nn.Module):
	...
	def forward(self, x, x0, cu_seqlens, position_ids):
...
class GPT(nn.Module):
	def __init__(self,h):
		...
		if h.rope_dims>0:
			...
			for block in self.blocks:
				...
				block.attn.rotary=Rotary(head_dim,base=h.rope_base,max_pos=h.train_seq_len,rope_dims=h.rope_dims)
	...
	def forward_logits(self, input_ids, cu_seqlens, position_ids):
		...
		for i in enc_iter:
			x = self.blocks[i](x, x0, cu_seqlens, position_ids)
			skips.append(x)
	...
	def forward(self, input_ids, target_ids, cu_seqlens, position_ids):
		logits = self.forward_logits(input_ids, cu_seqlens, position_ids)
...
def collect_hessians(model,train_loader,h,device,n_calibration_batches=64):
	...
	with torch.no_grad():
		for _ in range(n_calibration_batches):
			x, _, cu, pos = train_loader.next_batch(h.train_batch_tokens, h.grad_accum_steps)
			model.forward_logits(x, cu, pos)
...
def eval_val(h,device,val_data,model):
	...
	with torch.inference_mode():
		for batch_seq_start in range(seq_start,seq_end,local_batch_seqs):
			...
			cu, pos = compute_intra_doc_meta_gpu(x)
			with torch.autocast(device_type='cuda', dtype=torch.bfloat16, enabled=True):
				batch_loss = model(x, y, cu, pos).detach()
...
def eval_val_sliding(h,device,val_data,base_model,batch_seqs=32):
	...
	with torch.inference_mode():
		for bi in range(0,len(my_windows),batch_seqs):
			...
			cu, pos = compute_intra_doc_meta_gpu(x_batch)
			with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
				logits = logits_fn(x_batch, cu, pos)
	...
	for ci in range(num_chunks):
		...
		with torch.no_grad():
			for bi in range(0,len(my_windows),batch_seqs):
				...
				cu, pos = compute_intra_doc_meta_gpu(x_batch)
				with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
					logits = compiled_logits(x_batch, cu, pos)
				nll=F.cross_entropy(logits.reshape(-1,logits.size(-1)).float(),y_batch.reshape(-1),reduction='none').reshape(bsz,seq_len)
				for(i,ws)in enumerate(batch_ws):wlen=wlens[i];s=0 if ws==0 else context_size;scored_nll=nll[i,s:wlen].to(torch.float64);loss_sum+=scored_nll.sum();token_count+=float(wlen-s);tgt=y_batch[i,s:wlen];prev=x_batch[i,s:wlen];tb=val_data.base_bytes_lut[tgt].to(torch.float64);tb+=(val_data.has_leading_space_lut[tgt]&~val_data.is_boundary_token_lut[prev]).to(torch.float64);byte_count+=tb.sum()
...
def train_model(h,device,val_data):
	...
	def step_fn(step,lr_scale):
		...
		for micro_step in range(h.grad_accum_steps):
			...
			x, y, cu_seqlens, position_ids = train_loader.next_batch(
				h.train_batch_tokens, h.grad_accum_steps,
			)
			with torch.autocast(device_type='cuda', dtype=torch.bfloat16, enabled=True):
				loss = model(x, y, cu_seqlens, position_ids)
